...
class FriendRequestConsumer(AsyncWebsocketConsumer):
  ...
  def accept_request(self, request_id):
      try:
          friend_request = FriendRequest.objects.get(from_user=request_id, to_user=self.user)
  ...
  def get_friends(self):
      friendships1 = Friendship.objects.filter(user1=self.user).values_list('user2', flat=True)
      friendships2 = Friendship.objects.filter(user2=self.user).values_list('user1', flat=True)
