        ...
        .then((data) => {
            ...
            chatSocket.onmessage = function (e) {
                ...
                if (data.type === "connected_users") {
                    ...
                    if (connected_users[0] == username) document.getElementById("startRemoteGame").style.display = "block";
                    ...
                    if (data.action == "move_up") {
                        if (data.player == "1") player1.movespeed = -player1.curr_speedY;
                        else if (data.player == "2") player2.movespeed = -player2.curr_speedY;
                    } else if (data.action == "move_down") {
                        if (data.player == "1") player1.movespeed = player1.curr_speedY;
                        else if (data.player == "2") player2.movespeed = player2.curr_speedY;
                    } else if (data.action == "stop") {
                        if (data.player == "1") player1.movespeed = 0;
                        else if (data.player == "2") player2.movespeed = 0;
                    } else if (data.action == "power_up_used_speed") {
                        if (data.player == "1") player1.curr_speedY = playerSpeed_power;
                        else if (data.player == "2") player2.curr_speedY = playerSpeed_power;
                        speed_power_up_used = true;
                    } else if (data.action == "power_up_used_size") {
                        if (data.player == "1") player1.height = playerHeight_power;
                        else if (data.player == "2") player2.height = playerHeight_power;
                    ...
                    if (id !== 0) {
                        ...
                        if (username == connected_users[0]) alert(`${connected_users[1]} left the game!`);
                        else alert(`${connected_users[0]} left the game!`);
                        reset();
                    } else {
                        if (username == connected_users[0]) alert(`${connected_users[1]} left the lobby!`);
                        else alert(`${connected_users[0]} left the lobby!`);
                    }
                    return;
                }
...
function check_size_power_up() {
    if (size_power_up_used == false) {
        if (size_y < 0) size_y = boardHeight;
        else size_y -= 3;
...
function check_speed_power_up() {
    if (speed_power_up_used == false) {
        if (speed_y + 20 > boardHeight) speed_y = 0;
        else speed_y += 3;
...
function drawStar(ctx, x, y, size) {
    ...
    for (var i = 0; i < 5; i++) {
        ...
        ctx.lineTo(x + (size / 2) * Math.cos(((54 + i * 72) / 180) * Math.PI), y - (size / 2) * Math.sin(((54 + i * 72) / 180) * Math.PI));
    }
    ctx.closePath();
    ctx.lineWidth = 3;
    ctx.fill();
...
function check_and_change_dir_ball() {
    if (ball.y > boardHeight - ball.height / 2) {
        if (advanced_mode == true) ball.y = 0;
        else ball.speedY *= -1;
    }
    if (ball.y < 0) {
        if (advanced_mode == true) ball.y = boardHeight - ballHeight / 2;
        else ball.speedY *= -1;
    }
    if (ball.x <= player1.x + player1.width && ball.x + ball.width >= player1.x && ball.y + ball.height >= player1.y && ball.y <= player1.y + player1.height) {
    ...
    } else if (ball.x + ball.width >= player2.x && ball.x <= player2.x + player2.width && ball.y <= player2.y + player2.height && ball.y + ball.height >= player2.y) {
        if (ball.x > player2.x) {
...
function check_and_change_dir_player() {
    ...
    else if (player1.y > boardHeight - player1.height) player1.y = boardHeight - player1.height;
    else player1.y += player1.movespeed;
    context.fillRect(player1.x, player1.y, player1.width, player1.height);

    if (player2.y < 0) player2.y = 1;
    else if (player2.y > boardHeight - player2.height) player2.y = boardHeight - player2.height;
    else player2.y += player2.movespeed;
    context.fillRect(player2.x, player2.y, player2.width, player2.height);
}
...
function check_and_change_score() {
    ...
    if (score1 >= maxScore) {
        ...
        if (username == connected_users[0]) chatSocket.send(JSON.stringify({ type: "end_score", user_score: score1, opponent_score: score2, is_win: true, opponent: connected_users[1] }));
        else chatSocket.send(JSON.stringify({ type: "end_score", user_score: score2, opponent_score: score1, is_win: false, opponent: connected_users[0] }));
        return 1;
        ...
        if (username == connected_users[0]) chatSocket.send(JSON.stringify({ type: "end_score", user_score: score1, opponent_score: score2, is_win: false, opponent: connected_users[1] }));
        else chatSocket.send(JSON.stringify({ type: "end_score", user_score: score2, opponent_score: score1, is_win: true, opponent: connected_users[0] }));
        return 1;
    }
    return 0;
}
...
function check_input_froms() {
    if (maxScore === "" || ballSpeed === "") {
        ...
        if (maxScore > 12 || maxScore <= 3) return -1;
