...
export function start_game() {
    ...
    window.addEventListener("keydown", (event) => {
        if (event.code == "KeyW") {
            if (player1.y > 0) player1.movespeed = -player1.curr_speedY;
            else {
                player1.movespeed = 0;
                player1.y = 0;
            }
        } else if (event.code == "KeyS") {
            if (player1.y + player1.height < boardHeight) player1.movespeed = player1.curr_speedY;
            else {
                player1.movespeed = 0;
        ...
        if (event.code == "ArrowUp") {
            if (player2.y > 0) player2.movespeed = -player2.curr_speedY;
            else {
                player2.movespeed = 0;
                player2.y = 0;
            }
        } else if (event.code == "ArrowDown") {
            if (player2.y + player2.height < boardHeight) player2.movespeed = player2.curr_speedY;
            else {
                player2.movespeed = 0;
    ...
    function check_size_power_up() {
        ...
        if (size_power_up_used == false && areBallsTouching(ball.x, ball.y, ball.width / 2, size_x, size_y, 20)) {
            if (ball.speedX > 0) player1.height *= 2;
            else player2.height *= 2;
    ...
    function check_speed_power_up() {
        ...
        if (speed_power_up_used == false && areBallsTouching(ball.x, ball.y, ball.width / 2, speed_x, speed_y, 20)) {
            if (ball.speedX > 0) player1.curr_speedY *= 2.5;
            else player2.curr_speedY *= 2.5;
    ...
    function check_and_change_dir_ball() {
        if (ball.y > boardHeight - ball.height / 2) {
            if (advanced_mode == true) ball.y = 1;
            else ball.speedY *= -1;
        }
        if (ball.y < 0) {
            if (advanced_mode == true) ball.y = boardHeight - ballHeight / 2 - 1;
            else ball.speedY *= -1;
        }
